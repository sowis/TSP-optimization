...
const int ITERATE_COUNT = 100; // 반복 횟수
const int BAN_COUNT = 2; // 한 클러스터에서 한번에 떨어져나가는 노드 숫자
const bool MUTATION = true; // 돌연변이 여부
const int MUTATION_COUNT = 1; // 한 세대에서 돌연변이가 나타나는 개수

const string OUTPUT_FILE = "cluster_hamilton.tsv";
...
int main(void) {
	...
	double min_mst_sum = board.get_hamilton_sum();
	int gen = 0;
	printf("%d hamilton sum: %.2lf\n", gen, min_mst_sum);

	for (int i = 0; i < ITERATE_COUNT; ++i) {
		board.next(BAN_COUNT, MUTATION, MUTATION_COUNT);
		const double current_mst_sum = board.get_hamilton_sum();
